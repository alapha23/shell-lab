...
void eval(char *cmdline)
{
  ...
  if(if_builtin)	/* execute builtin cmd and return */
    return ;

    /* if it is not a cmd built-in */  
    pid = Fork();
    if(pid != 0)
    {
    ...
    {    
      setpgid(0 ,0);
      close(fd[1]);
      int buffer; 		 /* child is reading*/
      if( read(fd[0], &buffer, sizeof(buffer)) <0)
      {
        printf("Error read\n");
      }
      addjob(jobs, buffer, bg, cmdline);
 ...
 *    to the foreground job.If there is no foreground job, send it to all 
 *    background jobs.
 */
void sigint_handler(int sig)
{
  ...
  if(pid != 0) 		/* kill fg job*/
  {
    struct job_t *j = getjobjid(jobs, pid2jid(pid));
...
/*bgpid - Return PID array of background jobs, 0 if no background jobs */
pid_t* bgpid(struct job_t *jobs){
   int i,j=0;
   static int bgjobs[MAXJOBS];
